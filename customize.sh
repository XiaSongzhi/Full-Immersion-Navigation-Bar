  ...
  case "$hook_size" in
    ...
    *)
      if ! cat "$HOOK_APK" | pm install -r -d -S "$hook_size" >/dev/null 2>&1; then
        # v0.5.0 changes the signing key once. Remove only this hook package,
        # then retry so future updates keep the same signature.
        pm uninstall --user 0 com.iosbar.navhook >/dev/null 2>&1 || true
        cat "$HOOK_APK" | pm install -r -d -S "$hook_size" >/dev/null 2>&1 || \
          abort "SystemUI hook install failed. The module was not enabled."
      fi
      ui_print "LSPosed SystemUI hook and KernelSU WebUI installed"
      ;;
  esac
else
...
ui_print "Open this module in KernelSU Manager to adjust the bar."
ui_print "LSPosed scope: com.android.systemui"
exit 0
